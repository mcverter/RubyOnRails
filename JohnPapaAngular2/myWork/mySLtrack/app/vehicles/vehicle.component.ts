...
export class VehicleComponent implements CanDeactivate, OnDestroy, OnInit {
    ...
    cancel (showToast = true){
        ...
        if (showToast) {
            this._toastService.activate(`Cacnelled changes to ${this.vehicle.name}`)
        }
    }
        ...
        this._modalService.activate(msg).then((responseOK) => {
            if (responseOK) {
                ...
                this._vehicleService.deleteVehicle(this.vehicle)
                    .subscribe(() => {
                        this._toastService.activate(`Dleted ${this.vehicle.name}`);
                        this._gotoVehicles();
                    },
    ...
    save(){
        ...
        this._vehicleService.updateVehicle(this.vehicle)
            .subscribe(() => this._toastService.activate(`Successfully added ${this.vehicle.name}`))
    }
